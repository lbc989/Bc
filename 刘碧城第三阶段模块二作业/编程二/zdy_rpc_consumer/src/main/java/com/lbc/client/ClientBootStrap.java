...
public class ClientBootStrap implements Watcher {
    ...
    private List<String[]> getChildrenData(List<String> children) throws KeeperException, InterruptedException, UnsupportedEncodingException {
        ...
        for (String child : children) {
            ...
            //只连接其中一台机器
            minTakingTime = getOneNodeByTime(result,minTakingTime,nodeValue,nodeName);
		}
        return result;
    }
	
	private Integer getOneNodeByTime(List<String[]> result, Integer minTakingTime, String[] nodeValue, String[] nodeName) {
        //判断该节点是否为最小响应时间的节点
        if (minTakingTime == null) {
            minTakingTime = Integer.valueOf(nodeValue[1]);
            result.add(new String[]{nodeName[0], nodeName[1], nodeValue[0], nodeValue[1]});
        } else if (Integer.valueOf(nodeValue[1]) > minTakingTime) {
            return minTakingTime;
        } else if (Integer.valueOf(nodeValue[1]) < minTakingTime) {
            //取当前循环中节点的数据
            minTakingTime = Integer.valueOf(nodeValue[1]);
            result = new ArrayList<>();
            //将获取的节点数据放到list中，第一个值为ip，第二个值为端口号
            result.add(new String[]{nodeName[0], nodeName[1], nodeValue[0], nodeValue[1]});
        } else if (Integer.valueOf(nodeValue[1]) == minTakingTime) {
            // 随机取
